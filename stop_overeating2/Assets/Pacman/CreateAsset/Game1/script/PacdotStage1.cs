...
public class PacdotStage1 : MonoBehaviour {
	...
	private TMP_Text playerCaltext;
	...
	void Start(){
		...
		time_hpbar = (LevelSet.hp_bar_level)/(1000);
		singlebar = GameObject.FindWithTag ("SingleBar");
		Debug.Log (LevelSet.hp_bar_level);
	...
	public void TakeDamage ( int a_flag, float damage )
	{
		...
		if (a_flag > 0) {
			FindObjectOfType<Food_Count> ().FoodCount(a_flag);
		}
