...
{	
	...
	public float now_data = 0.0f;
	/*
	public void UpdateBar ( float currentValue, float maxValue )
	{


		float current_hp = 2.0f;

		// If the bar image is left unassigned, then return.
		Debug.Log("sttt");
		Debug.Log (current_hp + "current_hp");
		Debug.Log (currentValue + "currentValue");
		current_hp = currentValue + current_hp + now_data;
		Debug.Log (current_hp+ "current_hp");

		now_data = currentValue + now_data;
		...
		_currentFraction = current_hp / maxValue;

		// If the value is greater than 1 or less than 0, then fix the values to being min/max.
		if( _currentFraction < 0 || _currentFraction > 2 )
			_currentFraction = _currentFraction < 0 ? 0 : 2;
	...
	public void UpdateBar ( float currentValue, float maxValue )
	{
		...
		float current_hp = current_hp_public;

		// If the bar image is left unassigned, then return.
		//Debug.Log("sttt");
		//Debug.Log (current_hp + "current_hp");
		//Debug.Log (currentValue + "currentValue");
		current_hp = currentValue + current_hp/2 + now_data;
		//Debug.Log (current_hp+ "current_hp");
